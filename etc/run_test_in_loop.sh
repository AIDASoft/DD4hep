...
usage()  {
    ...
    echo "   -p         <number>      Parallelism flag";
...
PARSED_ARGUMENTS=$(getopt -a -n run_test_in_loop -o hg:Nl:R:p: --longoptions help,g4-data:,show-only,loops:,test-regex:,parallel: -- "[run_test_in_loop]" $*)
VALID_ARGUMENTS=$?
...
do
	...
	-R | --tests-regex)   test_name=${2};             shift 2 ;;
...
#
if test -z "${GEANT4_DATA_DIR}"; then
    export GEANT4_DATA_DIR=/scratch/online/frankm/SW/Geant4Data;
fi;
#
#
#
...
for i in $(seq 1 ${num_loop}); do
    ...
    exec_command ctest -j ${parallel} ${noop} -R ${test_name};
    if test "$?" = "0"; then
