...
\begin{itemize}
\item \texttt{CMake} version 3.14 or higher
...
\item {\textbf{<limitsets>}} contain parameters passed to Geant4:
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos,fontsize=\small]{xml}
<limits>
...
Segmentations define the user API to the low level interpretation of the energy deposits in a subdetector. For technical reasons and partial religious reasons the segmentation implementation is not part of the DD4hep toolkit, but an independent package called \texttt{DDSegmentation}. Though the usage is an integral part of DD4hep.
...
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos,fontsize=\small]{bash}
$> geoDisplay -arg [arg-value] <path to the \texttt{XML} file containing the detector description>
With -arg being (optionally):
...
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos,fontsize=\small]{bash}
    ...
    python <install>/bin/checkOverlaps.py --help
...
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos,fontsize=\small]{bash}
    ...
    Options:
      ...
      -x <double>, --vx=<double>            X-position of track origin vertex [requires 'full']
      -y <double>, --vy=<double>            Y-position of track origin vertex [requires 'full']
      -z <double>, --vz=<double>            Z-position of track origin vertex [requires 'full']
...
\end{minted}
\texttt{materialScan} uses the python bindings provided by Geant4 and may be not  always available. Alternatively the command \texttt{print\_materials} may be used, which does not use the python binding, but produces less pretty output.
...
To trigger the start of the ROOT interpreter, invoke the plugin. Additional arguments are 
...
ROOT command prompt. See the definition file \texttt{DDCore/include/DD4hep/DD4hepUI.h} for details.
An instance of the \texttt{DD4hepUI} class is generated and made available to ROOT by the global pointer
...
Useful to verify the parsed content.
...
of the detector description. Both plugin are very similar, once with emphasis on the
...
The DetElement instances may cache on demand the transformations of their ideal placements
...
To create shapes with the factory mechanism, the shape constructors as present in 
\texttt{DDCore/include/DD4hep/Shapes.h} must be met.
...
\end{minted}
$name$ is optional, $startphi=0$, $deltaphi=2\pi$ have default values if not explicitly set.
Otherwise the following pattern must be fulfilled (DEPRECATED):
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos,fontsize=\small]{bash}
   <some-tag name="my-polycone" rmin1="1*cm" rmax1="2.cm"  rmin1="2*cm" rmax1="3.cm" 
...
\subsection{Regular Trapezoid (TRD1) Construction}
...
\subsection{Irregular Trapezoid (TRD2) Construction}
...
Segmentation plugins are used to impose an artificial super-structure onto sensitive elements
such as a grid for pixel detectors etc. DD4hep supports several such segmentations, which 
are internally created with the plugin mechanism, which also allows to easily extend 
the existing palette of segmentations.
