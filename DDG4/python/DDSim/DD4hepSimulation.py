...
class DD4hepSimulation(object):
  ...
  def parseOptions(self, argv=None):
    ...
    parser.add_argument("--outputFile", "-O", action="store", default=self.outputFile,
                        help="Outputfile from the simulation: .slcio, edm4hep.root and .root output files are supported")
...
#      e4Out.RunHeader = self.meta.addParametersToRunHeader(self)
      e4Out.EventParametersString, e4Out.EventParametersInt, e4Out.EventParametersFloat = self.meta.parseEventParameters()
