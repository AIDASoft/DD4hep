...
void Geant4Output2EDM4hep::endRun(const G4Run* run)  {
  ...
  Geant4Context* workerCtx = context()->kernel().worker(Geant4Kernel::thread_self(), false).workerContext();
  {
    ...
    if(parameters) {
      if(m_runParameters) {
        printout(ERROR, "Geant4Output2EDM4hep" ,"Some other thread already stored RunParameters");
      } else {
...

  {
    ...
    if(parameters) {
      if(m_fileParameters) {
        printout(ERROR, "Geant4Output2EDM4hep" ,"Some other thread already stored FileParameters");
      } else {
