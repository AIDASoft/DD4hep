...
namespace dd4hep {
  ...
  /// Pretty print of solid attributes
  string toXMLSolid(const TGeoShape* shape, int precision)   {
    if ( !shape )  {
      return "<shape type=\"INVALID\"></shape>";
    }
    ...
    log.setf(ios::fixed, ios::floatfield);
    log << setprecision(precision);
    std::string lunit_nam = "mm";
    std::string aunit_nam = "deg";  
    std::string new_line = "";
    double lunit = units::mm;
    double aunit = units::deg;
    if ( cl == TGeoBBox::Class() )   {
      TGeoBBox* sh = (TGeoBBox*) shape;
      log << "<box lunit=\"" << lunit_nam << "\""
	  << " x=\"" << 2.0*sh->GetDX()/lunit << "\""
	  << " y=\"" << 2.0*sh->GetDY()/lunit << "\""
	  << " z=\"" << 2.0*sh->GetDZ()/lunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoHalfSpace::Class() ) {
      TGeoHalfSpace* sh = (TGeoHalfSpace*)(const_cast<TGeoShape*>(shape));
      ...
      log << "<halfspace lunit=\"" << lunit_nam << "\">" << new_line
	  << " <point x=\""  << pnt[0]/lunit << "\""
	  << " y=\"" << pnt[1]/lunit << "\""
	  << " z=\"" << pnt[2]/lunit << "\"/>" << new_line
	  << " <normal x=\"" << nrm[0]/lunit << "\""
	  << " y=\"" << nrm[1]/lunit << "\""
	  << " z=\"" << nrm[2]/lunit << "\"/>" << new_line
	  << "</halfspace> << new_line";
    }
	  ...
	  << " lunit=\""    << lunit_nam             << "\""
	  << " aunit=\""    << aunit_nam             << "\""
	  << " rmin=\"" << sh->GetRmin()/lunit << "\""
	  << " rmax=\"" << sh->GetRmax()/lunit << "\""
	  << " dz=\""   << sh->GetDz()/lunit << "\""
	  << " startphi=\"" << 0.0 << "\""
	  << " deltaphi=\"" << 360.0/aunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoTubeSeg::Class() ) {
      const TGeoTubeSeg* sh = (const TGeoTubeSeg*) shape;
	  ...
	  << " rmax=\"" << sh->GetRmax()/lunit << "\""
	  << " dz=\""   << sh->GetDz()/lunit << "\""
	  << " startphi=\"" << sh->GetPhi1()/aunit << "\""
	  << " deltaphi=\"" << (sh->GetPhi2() - sh->GetPhi1())/aunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoCtub::Class() ) {
      const TGeoCtub* sh = (const TGeoCtub*) shape;
	  ...
	  << " y1=\""     << sh->GetH1()/lunit     << "\""
	  << " y2=\""     << sh->GetH2()/lunit     << "\""
	  << " alpha1=\"" << sh->GetAlpha1()/aunit << "\""
	  << " alpha2=\"" << sh->GetAlpha2()/aunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoHype::Class() )   {
      const TGeoHype* sh = (const TGeoHype*) shape;
	  ...
	  << " rmin=\""   << sh->GetRmin()/lunit   << "\""
	  << " rmax=\""   << sh->GetRmax()/lunit   << "\""
	  << " inst=\""   << sh->GetStIn()/aunit   << "\""
	  << " outst=\""  << sh->GetStOut()/aunit  << "\""
	  << " z=\""      << sh->GetDz()/lunit     << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoPgon::Class() )   {
	  ...
	  << " aunit=\""  << aunit_nam             << "\""
	  << " startphi=\"" << sh->GetPhi1()/aunit << "\""
	  << " deltaphi=\"" << sh->GetDphi()/aunit << "\""
	  << " numsides=\"" << sh->GetNedges()     << "\">" << new_line;
      for(int i=0, n=sh->GetNz(); i<n; ++i)  {
        log << " <zplane z=\"" << sh->GetZ(i)/lunit 
	    << "\" rmin=\"" << sh->GetRmin(i) << "\""
	    << "\" rmax=\"" << sh->GetRmax(i) << "\"/>" << new_line;
      }
      log << "</polyhedra>" << new_line;
    }
    else if ( cl == TGeoPcon::Class() )  {
      const TGeoPcon* sh = (const TGeoPcon*) shape;
      log << "<polycone"
	  ...
	  << " aunit=\""  << aunit_nam             << "\""
	  << " startphi=\"" << sh->GetPhi1()/aunit << "\""
	  << " deltaphi=\"" << sh->GetDphi()/aunit << "\">" << new_line;
      for(int i=0, n=sh->GetNz(); i<n; ++i)  {
        log << " <zplane z=\"" << sh->GetZ(i)/lunit 
	    << "\" rmin=\"" << sh->GetRmin(i) << "\""
	    << "\" rmax=\"" << sh->GetRmax(i) << "\"/>" << new_line;
      }
      log << "</polycone>" << new_line;
    }
    else if ( cl == TGeoCone::Class() )  {
      const TGeoCone* sh = (const TGeoCone*) shape;
      log << "<cone"
	  << " lunit=\""  << lunit_nam             << "\""
	  << " aunit=\""  << aunit_nam             << "\""
	  << " rmin1=\""  << sh->GetRmin1()/lunit  << "\""
	  << " rmin2=\""  << sh->GetRmin2()/lunit  << "\""
	  << " rmax1=\""  << sh->GetRmax1()/lunit  << "\""
	  << " rmax2=\""  << sh->GetRmax2()/lunit  << "\""
	  << " z=\""      << sh->GetDz()/lunit     << "\""
	  << " startphi=\"" << 0.0/aunit           << "\""
	  << " deltaphi=\"" << 360.0/aunit         << "\""
	  << "/>" << new_line;
	  ...
	  << " z=\""      << sh->GetDz()/lunit     << "\""
	  << " startphi=\"" << sh->GetPhi1()/aunit << "\""
	  << " deltaphi=\"" << (sh->GetPhi1()-sh->GetPhi1())/aunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoParaboloid::Class() )  {
      const TGeoParaboloid* sh = (const TGeoParaboloid*) shape;
	  ...
	  << " startphi=\""   << sh->GetPhi1()/aunit << "\""
	  << " deltaphi=\""   << (sh->GetPhi1()-sh->GetPhi1())/aunit << "\""
	  << " starttheta=\"" << sh->GetTheta1()/aunit << "\""
	  << " deltatheta=\"" << (sh->GetTheta1()-sh->GetTheta1())/aunit << "\""
	  << "/>" << new_line;
    }
    else if ( cl == TGeoTorus::Class() )   {
      const TGeoTorus* sh = (const TGeoTorus*) shape;
	...
	log << " <twoDimVertex x=\"" << sh->GetX(i)/lunit << "\""
	    << "\" y=\"" << sh->GetY(i)/lunit << "\"/>" << new_line;
      }
      for (int i = 0; i < sh->GetNz(); i++) {
	log << " <section zOrder=\"" << i << "\""
	    << " scalingFactor=\"" << sh->GetScale(i) << "\""
	    << " zPosition=\"" << sh->GetZ(i)/lunit << "\""
	    << " xOffset=\""   << sh->GetXOffset(i)/lunit << "\""
	    << " yOffset=\""   << sh->GetYOffset(i)/lunit << "\"/>" << new_line;
      }
      log << "</xtru>" << new_line;
    }
    else if (shape->IsA() == TGeoCompositeShape::Class() )   {
